...
const App: React.FC = () => {
    ...
    return (
        <div className="flex flex-col min-h-screen font-sans bg-slate-100 text-slate-900">
            <header className="relative bg-slate-800 text-white p-2 text-center shadow-md z-10 sticky top-0">
                <div className="flex justify-center items-center">
                    ...
                    <button onClick={handleSwitchMode} className="bg-sky-600 hover:bg-sky-700 px-3 py-1 rounded-md text-sm transition-colors">{mode === 'user' ? '管理者へ' : 'ユーザーへ'}</button>
                </div>
            </header>
            ...
            <div className="fixed bottom-2 right-2 text-xs text-slate-400 z-10 bg-slate-100/50 backdrop-blur-sm px-2 py-1 rounded pointer-events-none">
                Career Consulting Support_v3.16 (Standard Modernization)
            </div>
        </div>
    );
}
