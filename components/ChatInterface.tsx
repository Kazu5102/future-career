...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ assistant, onExit }) => {
  ...
  useEffect(() => {
    const generateIntroduction = async () => {
      if (!API_KEY) {
        setError('APIキーが設定されていません。Vercelなどの環境変数を確認してください。');
        return;
      }
      ...
      try {
        ...
        setError('自己紹介の生成中にエラーが発生しました。APIキーまたはネットワーク接続を確認してください。');
        setMessages([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        ...
        setError('メッセージの送信中にエラーが発生しました。APIキーまたはネットワーク接続を確認してください。');
        // Keep user message on screen in case of error
