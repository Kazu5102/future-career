...
import { ChatMessage, MessageAuthor, StoredConversation, AnalysisData, AIType, IndividualAnalysisData, ConsultationEntry, SkillMatchingResult } from '../types';

// Initialize the AI client.
// The API key is expected to be available as an environment variable.
if (!process.env.API_KEY) {
    // This is a client-side check. In a real-world scenario, you'd want to handle this gracefully.
    // For this context, we assume the key is injected by the environment.
    console.error("API_KEY environment variable not set.");
}
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });


// --- Helper functions (from original proxy) ---
    ...
    try {
        ...
        const streamResult = await ai.models.generateContentStream({
            ...
            config: {
                ...
                temperature: aiType === 'dog' ? 0.8 : 0.6, //犬は創造的、人間は堅実に
                topK: 40,
                topP: 0.95,
            },
        });

        // Adapt the async iterator to a ReadableStream for the browser
        ...
        console.error("Streaming chat SDK error:", error);
...
`;
    try {
        ...
        // Ensure the parsed data conforms to the AnalysisData type
        // This is a basic check; more robust validation could be added here.
        if (
            !parsedData.keyMetrics ||
            !Array.isArray(parsedData.commonChallenges) ||
            !Array.isArray(parsedData.careerAspirations) ||
            !Array.isArray(parsedData.commonStrengths) ||
            !parsedData.overallInsights
        ) {
        ...
        if (error instanceof SyntaxError) {
             throw new Error("総合分析APIが不正なJSONを返しました。AIの応答形式に問題がある可能性があります。");
        }
...
`;
    ...
    try {
        ...
        if (!parsedData.userId || !Array.isArray(parsedData.keyThemes) || !Array.isArray(parsedData.consultations) || !parsedData.skillMatchingResult || !Array.isArray(parsedData.hiddenSkills)) {
             throw new Error("AI response does not match the required individual analysis schema.");
        }
        
        return parsedData as IndividualAnalysisData;

    } catch (error) {
        console.error("Error generating individual analysis:", error);
         if (error instanceof SyntaxError) {
             throw new Error("個別分析APIが不正なJSONを返しました。");
        }
...
`;
    ...
    try {
        ...
        // Basic validation
        if (!parsedData.recommendedRoles || !parsedData.skillsToDevelop) {
            throw new Error("AI response does not match the required skill matching schema.");
        }
        
        return parsedData as SkillMatchingResult;

    } catch (error) {
        console.error("Error generating skill matching analysis:", error);
        if (error instanceof SyntaxError) {
            throw new Error("スキルマッチングAPIが不正なJSONを返しました。");
        }
